    ...
    if (which == COMBINE){
      ...
      if (true){
        if (id >= 0){ 
          ...
          vec.addRow(*output_, id, dict_weight);
        }
      } else {
        if (id >= 0){
...
void FastText::saveNgramVectors(std::string prefix) {
  ...
  std::cerr << "Saving dictionary to file: " << fndict << std::endl;
  std::ofstream ofs_dict(fndict);
  if (!ofs_dict.is_open()) {
    std::cerr << "Error opening file for saving dict." << std::endl;
    exit(EXIT_FAILURE);
  }
  for (int32_t i = 0; i < dict_->nwords(); i++) {
    std::string word = dict_->getWord(i);
    ofs_dict << word << std::endl;
  }
  ///////////////////////////////////////
  ...
  ofs_in.close();
  std::cerr << "Writing output_ to file " << std::endl;
  std::ofstream ofs3(prefix + ".out");
  if (!ofs3.is_open()) {
    std::cerr << "Error opening file for saving vectors." << std::endl;
    exit(EXIT_FAILURE);
  }  
...
void FastText::loadModel(std::istream& in) {
  ...
  model_ = std::make_shared<Model>(input_, output_, input2_, output2_, inputvar_, input2var_, outputvar_, output2var_, args_, 0, dict_->nwords());
...
void FastText::wordVectors(int which_emb, float beta) {
  std::string word;
  Vector vec(args_->dim);
  while (std::cin >> word) {
    getVector(vec, word, which_emb, beta);
    std::cout << word << " " << vec << std::endl;
  }
}
...
void FastText::printWordVectors(int which_emb, float beta) {
  wordVectors(which_emb, beta);
}
...
void FastText::trainThread(int32_t threadId) {
  ...
  utils::seek(ifs, threadId * utils::size(ifs) / args_->thread);
    ...
    Model model(input_, output_, input2_, output2_,
              inputvar_, input2var_, outputvar_, output2var_, 
              args_, threadId, dict_->nwords());
...
void FastText::train(std::shared_ptr<Args> args) {
  ...
  ifs.close();
  // For initialization of variance
  real logvar = log(args_->var_scale);
